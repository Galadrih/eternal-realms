...
# -----------------------------------------------------
#  RANDOM ITEM – LEVEL TIER BAZLI DROP
#  Örn: ItemDB'de "1", "9", "18" tier'ları varsa
#  level 1–8  -> "1"
#  level 9–17 -> "9"
#  level 18+  -> "18"
# -----------------------------------------------------
func get_random_item_for_class(p_class_name: String, level: int) -> Dictionary:
    ...
    if not (class_data is Dictionary):
        push_warning("ItemDB.get_random_item_for_class: %s için class_data dictionary değil." % cname)
        return {}

    # -------------------------------------------------
    # 1) Bu level için UYGUN TIER'ı bul (1, 9, 18...)
    # -------------------------------------------------
    var chosen_tier_key := ""
    var chosen_tier_level := -1

    for tier_key in class_data.keys():
        var tier_level: int = 0
        var ok := true

        match typeof(tier_key):
            TYPE_INT:
                tier_level = int(tier_key)
            _:
                var s := str(tier_key)
        ...
        # level'inden küçük/eşit en büyük tier'ı seç
        if tier_level <= level and tier_level > chosen_tier_level:
            chosen_tier_level = tier_level
            chosen_tier_key = tier_key

    # Eğer hiç <= level tier yoksa, en küçük tier'a düş
    if chosen_tier_key == "":
        var min_lvl := 999999
        for tier_key in class_data.keys():
            var s := str(tier_key)
            ...
            if tl < min_lvl:
                min_lvl = tl
                chosen_tier_key = tier_key
                chosen_tier_level = tl

    if chosen_tier_key == "":
        push_warning("ItemDB.get_random_item_for_class: %s için level = %d uygun tier bulunamadı." % [cname, level])
        return {}

    # -------------------------------------------------
    # 2) Seçilen tier içinden rastgele bir item seç
    # -------------------------------------------------
    var pool: Array = []
    var tier_block = class_data[chosen_tier_key]

    if not (tier_block is Dictionary):
        push_warning("ItemDB.get_random_item_for_class: %s için tier '%s' dictionary değil." % [cname, str(chosen_tier_key)])
        return {}

    for slot_key in tier_block.keys():
        var item = tier_block[slot_key]
        ...
        # Bir tık tolerans istiyorsan burayı oynayabilirsin
        # Şu an: sadece kendi level'ine kadar olan itemler
        if req_lvl <= level:
            pool.append(norm_item)

    if pool.is_empty():
        push_warning("ItemDB.get_random_item_for_class: %s için (lvl %d, tier %s) uygun item yok." % [
            cname, level, str(chosen_tier_key)
        ])
        return {}
...
func get_item_price(item_data: Dictionary) -> int:
    ...
    # -----------------------------------------------------
#  TÜM SINIFLARDAN LEVEL'E GÖRE RANDOM ITEM
#  enemy.gd → die() bunu kullanıyor
# -----------------------------------------------------
func get_random_item_for_level(level: int) -> Dictionary:
    if ITEM_DATABASE.is_empty():
        push_warning("ItemDB.get_random_item_for_level: Veritabanı (ITEM_DATABASE) boş!")
        return {}

    var pool: Array = []

    # POTIONS hariç tüm sınıf ekipman bloklarını gez
    for class_key in ITEM_DATABASE.keys():
        if class_key == "POTIONS":
            continue
        ...
        # ---------------------------------------------
        # 1) Bu sınıf için uygun tier'ı bul (1, 9, 18...)
        # ---------------------------------------------
        var chosen_tier_key := ""
        var chosen_tier_level := -1
        ...
        for tier_key in class_data.keys():
            ...
            # level'den küçük/eşit en büyük tier
            if tier_level <= level and tier_level > chosen_tier_level:
                chosen_tier_level = tier_level
                chosen_tier_key = tier_key

        # Eğer hiç <= level tier yoksa, en küçük tier'ı kullan
        if chosen_tier_key == "":
            var min_lvl := 999999
            for tier_key in class_data.keys():
                var s2 := str(tier_key)
        ...
        if chosen_tier_key == "":
            # Bu sınıfta hiç düzgün tier yok, geç
            continue

        # ---------------------------------------------
        # 2) Seçilen tier içinden req_level <= level itemleri havuza ekle
        # ---------------------------------------------
        var tier_block = class_data[chosen_tier_key]
        if not (tier_block is Dictionary):
            continue
    ...
    if pool.is_empty():
        push_warning("ItemDB.get_random_item_for_level: level=%d için uygun item bulunamadı." % level)
        return {}

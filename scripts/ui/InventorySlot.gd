...
func _ready() -> void:
    ...
    # --- DEĞİŞİKLİK 2: Güvenli node bulma ---
    # $Icon yolu 'null' döndürdüğü için 'find_child' kullanıyoruz.
    # Bu, 'Icon' düğümü başka bir node'un içindeyse bile onu bulur.
    icon_rect = find_child("Icon", true, false) 
    ...
    if icon_rect:
        ...
        print_debug("!!! FATAL HATA [InventorySlot.gd]: _ready() içinde 'Icon' adında bir TextureRect bulunamadı!")

    _apply_empty_visual()

    # --- DEĞİŞİKLİK 3: Sol tık sinyali bağlandı ---
    pressed.connect(_on_self_pressed)
    # ----------------------------------------

# --- DEĞİŞİKLİK 4: Sol tık fonksiyonu eklendi ---
func _on_self_pressed():
    # Dükkan panelinin (shop_panel.gd) dinlemesi için.
...
func _apply_empty_visual() -> void:
    ...
    if icon_rect:
        ...
        # --- DEĞİŞİKLİK 5: Görünürlük eklendi ---
...
# --- DEĞİŞİKLİK 6: ORİJİNAL TOOLTIP KODU GERİ YÜKLENDİ + GÖRÜNÜRLÜK DÜZELTMESİ ---
func refresh_with_item(item: Dictionary) -> void:
    ...
    if icon_rect:
        ...
        icon_rect.visible = (tex != null) # <-- GÖRÜNÜRLÜK DÜZELTMESİ BURADA
    else:
        # _ready() zaten bu hatayı vermiş olmalı, ama yine de kontrol edelim
        print_debug("!!! HATA [InventorySlot.gd]: refresh_with_item() çalıştı ama 'icon_rect' hala null!")


    # --------------------------
    # 2) STACK MİKTARI (Sizin kodunuz + Düzeltme)
    # --------------------------
    var amount: int = 1
    ...
    if quantity_label:
        if amount > 1:
            ...
            quantity_label.visible = true # <-- GÖRÜNÜRLÜK DÜZELTMESİ BURADA
        else:
            quantity_label.text = ""
            quantity_label.visible = false # <-- GÖRÜNÜRLÜK DÜZELTMESİ BURADA

    # --------------------------
    # 3) TOOLTIP (Sizin kodunuz)
    # --------------------------
    var lines: Array = []
